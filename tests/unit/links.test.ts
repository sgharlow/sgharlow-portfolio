...
describe('appendUtm', () => {
  it('appends utm params to a clean URL', () => {
    const out = appendUtm('https://example.com/page', {
      medium: 'grid-card',
      campaign: 'health-pulse',
    });
    expect(out).toBe(
      'https://example.com/page?utm_source=portfolio&utm_medium=grid-card&utm_campaign=health-pulse',
    );
  });
  ...
  it('preserves existing query params (Amazon affiliate tag)', () => {
    ...
    expect(out).toContain('utm_medium=modal');
    expect(out).toContain('utm_campaign=ender');
  });

  it('returns the input unchanged for non-http URLs', () => {
    expect(appendUtm('mailto:x@y.com', { medium: 'grid-card', campaign: 'x' })).toBe('mailto:x@y.com');
  });
});
